...
> научить. В лучшем случае это способ натренировать.
...
> *Перевод:* Завод построили не для того, чтобы выходить в ноль.
> UniCo занимается бизнесом не ради того, чтобы выходить в ноль.
> Компания существует, чтобы делать деньги. Теперь я это вижу.
...
> *Перевод:* Те привычные показатели, которыми вы выражаете цель,
> плохо ложатся на ежедневную работу производственной
> организации. Собственно, поэтому я и разработал другой набор
> метрик.
...
- **Throughput** — «the rate at which the system generates money
  through sales» (рус.: «скорость, с которой система производит
  деньги через продажи»).
- **Inventory** — «all the money that the system has invested in
  purchasing things which it intends to sell» (рус.: «все деньги,
  которые система вложила в покупку того, что собирается
  продать»).
- **Operating Expense** — «all the money the system spends in order
  to turn inventory into throughput» (рус.: «все деньги, которые
  система тратит, чтобы превратить inventory в throughput»).
...
> *Перевод:* Моя максимальная скорость — это скорость, с которой
> идёт Herbie. Моя скорость — это throughput. Скорость Herbie
> задаёт мою. Значит, Herbie на самом деле и определяет
...
> *Перевод:* Если ограничение не физическое — этот шаг
> бессмыслен.
