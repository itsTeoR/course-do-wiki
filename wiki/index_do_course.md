...
отдельно в [wiki/README.md](README.md); здесь её нет намеренно, чтобы
рабочий экран не превращался в каталог.

## Гипотезы курса

Рабочие гипотезы, под которыми собирается курс. Меняются по ходу
работы. В статьях wiki отражены только в опциональной нижней
секции «В линзе гипотез курса» (формат — в CLAUDE.md, раздел
«Секция ‘В линзе гипотез курса’»); основное тело статей про
гипотезы курса не знает.

Канонические имена `H1`, `H2`, … используются как подзаголовки в
секциях-линзах. Не меняй номера без явной причины: изменение
номера требует синхронной правки всех статей с линзой.

### H1 — Как наконец начать делать дела

Тема курса (рабочая формулировка). Участник чувствует, что у него
есть дело, которое он давно не начинает или не может довести — и
курс про то, что с этим делать.

### H2 — Самоконтроль как J-адаптированная технология инициирования

Самоконтроль — одна из технологий инициирования действия, хорошо
описанная и адаптированная под J-людей (в терминах Юнга / MBTI —
judging attitude). Для P-людей (perceiving attitude) эта
технология работает плохо или частично, но почти вся литература
по продуктивности написана в J-логике. Курс ищет и собирает
технологии инициирования действия, работающие для P-модальности
— их должно быть много, и их надо искать и исследовать.

## Статьи с линзой гипотез

Реестр статей wiki, в которые добавлена нижняя секция «В линзе
гипотез курса». Пополняется после каждой команды «добавь линзу
к [статья]».

- [[bridges/affordable_loss_karma_yoga]] — H1, H2
- [[bridges/via_negativa_goldratt_gallwey]] — H1, H2
- [[tensions/effectuation_vs_goal_setting]] — H1, H2
- [[tensions/environment_design_vs_temperament]] — H1, H2
...
### Кандидаты
- Via negativa vs via positiva (Taleb vs Fogg): Taleb настаивает,
  что позитивный совет — шарлатанский, сначала надо убрать
  (iatrogenic risk); Fogg вся механика построена как чистое
  добавление — якорь, поведение, празднование.
...
- Среда курса по умолчанию проектируется в J-модусе (расписание,
  ...
  курса.
- Одна универсальная среда курса (в расчёте на B=MAP) или несколько
  траекторий под разные типы/темпераменты (и тогда заранее
  диагностировать участников)? Вопрос из
  [[tensions/environment_design_vs_temperament]], перенесён сюда
  как дизайн-решение курса.
